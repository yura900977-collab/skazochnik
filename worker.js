  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/generate') {
      ...
      if (type === 'letter') {
        ...
        prompt = `Напиши уникальную детскую сказку на русском языке. Главный герой — ${genderWord} по имени ${name}, ${age} лет, увлекается: ${interests || 'приключения'}. Настроение: ${moodMap[mood] || 'весёлая'}. Длина: ${lengthMap[length] || '500–800 слов'}. Формат: первая строка — заголовок в формате "# Название сказки", затем текст с абзацами. Сказка должна быть доброй, безопасной для детей, с моралью.`;
      }
