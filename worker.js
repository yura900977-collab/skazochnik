  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/generate') {
      ...
      if (type === 'letter') {
        ...
        prompt = `Напиши уникальную детскую сказку на русском языке. Главный герой — ${genderWord} по имени ${name}, ${age} лет, увлекается: ${interests || 'приключения'}. Настроение: ${moodMap[mood] || 'весёлая'}. Длина сказки: ${lengthMap[length] || lengthMap.short} — это критически важно, не выходи за рамки. Формат: первая строка — заголовок в формате "# Название сказки", затем текст с абзацами. Сказка должна быть доброй, безопасной для детей, с моралью.`;
      }
